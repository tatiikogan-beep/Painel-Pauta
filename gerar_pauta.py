...
COORDS={
    ...
    'JULIANA MIRELLA ALVES RODRIGUES':{'dark':'145A32','light':'D5F5E3',
        'adv':['ARTHUR MASSARI','DANIEL BARROS DE OLIVEIRA','EVILANY GABRIELA BRAGA PONTES',
               'FRANCOISE CATHERINE SOUZA ALVES','GIOVANNA CAMPOS PEREIRA','KELIANE DE OLIVEIRA',
               'LETICIA OLIVEIRA DA SILVA','MATHEUS CAVALCANTI DE ARAUJO',
               'MONIQUE DE KAROLIN SILVA DA COSTA','NATALIA PAIVA DE PAULA',
               'ROBERTA RAYANNE VASCONCELOS BOTO','TATIANE CARMO SANTA ROSA',
    ...
    'SUZANA MARIA CAMPOS MARANHÃO DE LIMA':{'dark':'922B21','light':'FADBD8',
        'adv':['GIOVANNA CESAR FERREIRA','GUSTAVO LOPES ALENCAR FILHO']},
    'YURI ALVES BARROS DOS SANTOS':{'dark':'154360','light':'D6EAF8',
        'adv':['JULIA MENEZES MORGADO']},
...
def gerar_pauta(src_new_bytes: bytes, src_old_bytes: bytes=None):
    ...
    for cs,label,span in [(2,'COORDENADOR',5),(7,'TOTAL',1),(8,'PRES.',1),(9,'VIRT.',1),(10,'HÍBR.',1),(11,'CTRL.',1),(12,'SUPORTE',1),(13,'C/ACOMP.',1)]:
        ...
        c.fill=PatternFill('solid',start_color=MED_BLUE)
        c.alignment=Alignment(horizontal='center',vertical='center',wrap_text=True); c.border=tb()
        for _sc in range(cs+1,cs+span):
            _c2=ws_dg.cell(r,_sc); _c2.fill=PatternFill('solid',start_color=MED_BLUE); _c2.border=tb()
    ...
    for idx,(coord,info) in enumerate(COORDS.items()):
        ...
        c.alignment=Alignment(horizontal='left',vertical='center'); c.border=tb()
        for _sc in range(3,7):
            _c2=ws_dg.cell(r,_sc); _c2.fill=PatternFill('solid',start_color=alt); _c2.border=tb()
        for col,cond in [
    ...
    c.alignment=Alignment(horizontal='center',vertical='center'); c.border=tb()
    for _sc in range(3,7):
        _c2=ws_dg.cell(r,_sc); _c2.fill=PatternFill('solid',start_color=NAVY); _c2.border=tb()
